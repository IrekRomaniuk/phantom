"""
Get predefined botnet report and open ticket for confidence > 3
"""
            ...
            if type(data) is dict:
                ...
                for entry in data:
                    ...
                    if int(entry['confidence']) > int(CONFIDENCE):                                                
                        result = result + '\n' + '{:15} {:25} {:40}'.format(entry['src'], entry['srcuser'], entry['description'][:80])
...
def send_message_1(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None):
    ...
    phantom.act("send message", parameters=parameters, assets=['niuk'], name="send_message_1", parent_action=action)    
...
Reboot affected host(s) as needed to fully clear quarantines malware
    ...
    parameters.append({
        ...
        'table': "incident",
        'fields': "{\"urgency\": \"1\", \"impact\": \"3\",\"comments\": \"Playbook name: %s\", \"assignment_group\": \"Help Desk\", \"company\": \"Company\", \"caller_id\": \"phantom\", \"u_affected_business_service\": \"Security\", \"u_symptom\": \"Infosec Incident\"}" %playbook_name,
        'vault_id': "",
    })
